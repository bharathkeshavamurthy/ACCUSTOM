...
def static_deployment_channel(_uav, _los):
    """
    MU-MIMO Cluster-UAV channel generation, with both large- and small-scale fading, for static UAV deployments
    """
...
print('[INFO] StaticUAVEvaluations core_operations: Rotary-wing UAV mobility model simulation parameters are - '
...
for uav in uavs:
    uav['los_channel'] = np.array(static_deployment_channel(uav, True), dtype=np.complex128)
    uav['nlos_channel'] = np.array(static_deployment_channel(uav, False), dtype=np.complex128)
