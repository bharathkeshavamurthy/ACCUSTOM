...
DEPLOYMENT MECHANISM:
    a. Rx signal power based Voronoi decomposition creates clusters of GNs, with a UAV serving each cluster;
...
def voronoi_channel_1(_gn, _uav):
    """
    Voronoi MU-MIMO Cluster-UAV channel generation considering both large- and small-scale fading statistics

    NOTE: LoS links are only modeled here between the GN and the UAV for Rx power based Voronoi decomposition.
    """
    _h_matrix = [[] for _ in range(n_a_u)]

    for _a_u in range(n_a_u):
        _a_gu = angle(_gn['voxel'], _uav['serv_voxel'])
        _d_gu, _k_factor = distance_3d(_gn['voxel'], _uav['serv_voxel']), k_1 * np.exp(k_2 * _a_gu)

        _beta = beta_0 * (_d_gu ** -alpha)
...
def voronoi_channel_2(_uav, _los):
    """
    Voronoi MU-MIMO Cluster-UAV channel generation considering both large- and small-scale fading statistics
    """
    _h_matrix = [[] for _ in range(n_a_u)]
...
# Simulation begins...
print('[INFO] RxPowerVoronoiEvaluations core_operations: Setting up the simulation for an Rx pwr. Voronoi deployment!')
...
# NOTE: The objective function for Voronoi decomposition considers the Rx powers assuming LoS links...
obj_fn = lambda __x, __w, __gn, __uav: sum([abs((np.sqrt(tx_p) * (voronoi_channel_1(__gn, __uav)[__a_u, :] @
...
while sum([abs(curr_objs[_gn] - prev_objs[_gn]) <= eps for _gn in range(n_g)]) < n_g:
    ...
    for uav in uavs:
        ...
        for g, gn in enumerate(gns_vor):
            ...
            obj = obj_fn(ref_sig_vec, w_vec, gn, uav)
            if obj <= min([obj_fn(ref_sig_vec, w_vecs[_uav['id']],
                                  gn, _uav) for _uav in uavs if uav['id'] != _uav['id']]):
                curr_objs[gn['id']] = obj
                uav['gns'].append(gn)
                del gns_vor[g]
...
for uav in uavs:
    uav['los_channel'] = np.array(voronoi_channel_2(uav, True), dtype=np.complex128)
    uav['nlos_channel'] = np.array(voronoi_channel_2(uav, False), dtype=np.complex128)
...
for uav in uavs:
    ...
    for gn in uav['gns']:  # GNs served by 'uav'
        ...
        if serv_time > avail_serv_time:
            print('[WARN] RxPowerVoronoiEvaluations core_operations: GN service time exceeds the total available '
...
# Simulation ends...
print('[INFO] RxPowerVoronoiEvaluations core_operations: Finished the simulation for an Rx pwr. Voronoi deployment!')
