...
pi, ld, it_max, a_gd, b_gd, df_gd = np.pi, constants.speed_of_light / 1e9, 1000, 0.99, 0.05, 0.99
g, n_u, n_g, n_c, n_a_u, n_a_g, wgt_uav, v_h_max, v_v_max = constants.g, 6, 36, 6, 16, 4, 80, 50, 50
temp, k_1, k_2, z_1, z_2, alpha, alpha_, kappa, bw = 300, 1, np.log(100) / 90, 9.61, 0.16, 2, 2.8, 0.2, 5e6
t_max, x_max, y_max, z_max, x_d, y_d, z_d, h_g, h_u, v_stp = 3000, 3000, 3000, 150, 10, 10, 10, None, None, 0.1
r_tw, delta, rho, rtr_rad, inc_corr, fp_area, n_bld, bld_len, rpm = 1, 0.012, 1.225, 0.4, 0.1, 0.0302, 8, 0.0157, 5730
v_min, v_max, v_p_min, tx_p, beta_0, w_var = 0, 50, 16.8, dbm_watts(23), db_watts(20), constants.Boltzmann * temp * bw
...
def igd_channel_1(_p_m, _uav):
    """
    IGD distributed SU-MIMO GN-UAV channel generation considering only large-scale fading statistics (no prob. LoS-NLoS)
...
def igd_channel_2(_uav, _los):
    """
    IGD MU-MIMO Cluster-UAV channel generation considering both large- and small-scale fading statistics
    """
...
assert n_c == n_u, 'The number of UAVs should be equal to the number of GN clusters for this IGD deployment!'
...
for uav in uavs:
    ...
    for gn in uav['gns']:
        ...
        for it in range(it_max):
            uav['channel'] = np.array(igd_channel_1(p_m, uav), dtype=np.complex128)
...
for uav in uavs:
    uav['los_channel'] = np.array(igd_channel_2(uav, True), dtype=np.complex128)
    uav['nlos_channel'] = np.array(igd_channel_2(uav, False), dtype=np.complex128)
...
So, here in this IGD framework, to maintain consistency across comparisons, if a UAV nears a collision 
